...
export default function TokenLogin({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #00a884, #075e54)" }}>
      <div className="w-full max-w-sm mx-4 rounded-2xl shadow-2xl p-8" style={{ background: "#fff" }}>
        <div className="flex justify-center mb-6">
          <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: "#e8f5e9" }}>
            <Lock size={28} style={{ color: "#00a884" }} />
          </div>
        </div>
        <h1 className="text-2xl font-bold text-center mb-1" style={{ color: "#111b21" }}>ATOZ Taxation</h1>
        <p className="text-sm text-center mb-6" style={{ color: "#667781" }}>Console access token daalo</p>
        <input type="password" value={token} onChange={e => { setVal(e.target.value); setError(""); }}
          ...
          style={{ background: "#f0f2f5", border: error ? "2px solid #e53e3e" : "2px solid transparent", color: "#111b21" }} />
        ...
        <p className="text-xs text-center mt-4" style={{ color: "#667781" }}>Token apne admin se maango</p>
      </div>
    </div>
  );
}
